---
name: resilience-testing
description: OWASP, reconnaissance, exploitation, rapport
author: PhoenixProject
version: 1.0.0
created: 2026-02-18
last_updated: 2026-02-23
---
# Pentest Workflow
## Phase 1 : Reconnaissance
```
Passive:
- whois, DNS records, certificate transparency
- Shodan, Censys, Google Dorks
- Social engineering (LinkedIn, GitHub repos)
Active:
- nmap -sV -sC -O target
- gobuster dir -u https://target -w wordlist.txt
- subfinder -d target.com
```
## Phase 2 : Analyse
```
- Nikto scan
- ffuf fuzzing (directories, parameters)
- Burp Suite interception
- SQLMap pour injection
- Nuclei templates
```
## Phase 3 : Exploitation
```
OWASP Top 10 checklist:
- A01: Broken Access Control (IDOR, privilege escalation)
- A03: Injection (SQLi, XSS, SSTI, command injection)
- A07: Auth Failures (brute force, session hijacking)
- A10: SSRF (cloud metadata, internal services)
```
## Phase 4 : Rapport
```
Pour chaque vulnerabilite:
- Titre
- Severite (CVSS 3.1)
- Description
- Preuve de concept (PoC)
- Impact potentiel
- Remediation recommandee
```
## Ethique
- Toujours avoir une autorisation ecrite (scope)
- Ne jamais exploiter au-dela du scope
- Rapporter immediatement les vulnerabilites critiques
- Proteger les donnees sensibles decouvertes
